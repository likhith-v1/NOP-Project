...
class LipschitzMomentumOptimizer(Optimizer):
    ...
    def __init__(
        ...
        self.beta_log: List[float] = []
        self.lipschitz_log: List[float] = []
        self.lambda_log: List[float] = []
    ...
    def _hvp(
        ...
        return [
            h.detach() if h is not None else torch.zeros_like(p)
    ...
    @staticmethod
    def _compute_beta(
        L_t: float, beta_min: float, beta_max: float, eps: float
    ) -> float:
        """β_t = clip(1 - 1/√(L_t + ε), β_min, β_max)"""
    ...
    def _update_lm_damping(self, current_loss: float, group: dict) -> float:
        if self._prev_loss is not None:
            if current_loss > self._prev_loss:
                self._lm_lambda = min(
                    self._lm_lambda * group["lm_lambda_up"], group["lm_lambda_max"]
                )
            else:
                self._lm_lambda = max(
                    self._lm_lambda / group["lm_lambda_down"], group["lm_lambda_min"]
                )
    ...
    def step(
        ...
        loss_val = (
            ...
            else (loss.item() if loss is not None else None)
        )
        ...
        for group in self.param_groups:
            params_with_grad = [
                p for p in group["params"] if p.grad is not None and p.requires_grad
            ]
            if not params_with_grad:
                continue

            effective_lr = (
                self._update_lm_damping(loss_val, group)
                if loss_val is not None
                else group["lr"]
            )

            L_t = self._estimate_lipschitz_from_grads(params_with_grad, group)
            beta_t = self._compute_beta(
                L_t, group["beta_min"], group["beta_max"], group["eps"]
            )
